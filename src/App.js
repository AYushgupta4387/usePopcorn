...
export default function App() {
  ...
  // fetch(`http://www.omdbapi.com/?apikey=${KEY}&s=race`)
  //   .then((res) => res.json())
  ...
  const handleSelectMovie = function (id) {
    setSelectedId(id === selectedId ? null : id); // This closes the movie details if we click on the same movie twice
  };
...
const Search = function ({ query, setQuery }) {
  ...
  // Adding functionality - To foucs on the search input when we press enter
  ...
  return (
    <input
      ...
      ref={inputEl} // ref gets added only after the DOM has loaded
    />
  );
};
...
const MovieDetails = function ({
  ...
  // Adding functionality - On pressing Esc key, the current movie details gets removed.
  ...
  useEffect(
    function () {
      ...
      return function () {
        ...
        console.log(`Clean up function for the movie ${title}`); // Clean up function runs after the component unmounts or rerenders, yet it will remember the previous title. Due to closures.
      };
    },
    [title]
  );
